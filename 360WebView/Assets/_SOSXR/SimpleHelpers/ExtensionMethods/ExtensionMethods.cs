...
namespace mrstruijk.Extensions
{
    public static class ExtensionMethods
    {
        ...
        /// </summary>
        /// <returns></returns>
        public static Vector3 CalculateDisplacement(this Transform originTrans, Transform targetTrans,
                                                    Vector3 axisToUse)
        {
        ...
        /// <returns></returns>
        public static float CalculateLookAtDotProduct(this Transform transformOne, Transform lookAtTarget,
                                                      bool remap = false)
        {
            ...
            var dotProduct =
                Vector3.Dot(direction,
                    transformOne.forward.normalized); // -1 to 1, with 1 looking directly at, -1 looking directly away
        ...
        /// <returns></returns>
        public static string ExtractedFileNameFromPath(this string fullPath, string removeBeforeCharacter = "/",
        ...
        public static void DrawCircle(this GameObject gameObject, float radius = 1f, float lineWidth = 0.025f,
        ...
        /// <returns></returns>
        public static float RoundCorrectly(this float originalValue, int decimals = 2)
        {
            ...
            var originalAsDecimal =
                (decimal) originalValue; // Float needs to be cast to decimal. Does not work properly when using double.

            const MidpointRounding
                midwayRounding =
                    MidpointRounding
                        .AwayFromZero; // As per: https://stackoverflow.com/questions/37290845/incorrect-result-of-math-round-function-in-vb-net  /// And: https://docs.microsoft.com/en-us/dotnet/api/system.midpointrounding?redirectedfrom=MSDN&view=net-6.0
        ...
        public static double RoundCorrectly(this double originalValue, int decimals = 2)
        {
        ...
        /// <param name="decimals"></param>
        /// <returns></returns>
        public static Vector2 RoundCorrectly(this Vector2 originalValue, int decimals = 2)
        {
        ...
        /// <param name="decimals"></param>
        /// <returns></returns>
        public static Vector3 RoundCorrectly(this Vector3 originalValue, int decimals = 2)
        {
            var rounded = new Vector3
            {
                x = RoundCorrectly(originalValue.x, decimals),
        ...
        public static IEnumerable<UnityAction> GetPersistentCallersFromUnityEvent(this UnityEvent unityEvent)
        {
            ...
            for (var i = 0; i < unityEvent.GetPersistentEventCount(); i++)
            {
                ...
                if (methodInfo != null)
                {
                    ...
                    {
                        Log.Warning("GetPersistentCallersFromUnityEvent",
                            "Cannot handle multiple parameters. Make sure to add", methodName, "from", target.name,
                    ...
                    if (method == null)
                    {
                        Log.Warning("GetPersistentCallersFromUnityEvent", "Could not create delegate for", methodName,
                            "on", target, "with Type", targetType);

                        continue;
                    }

                    Log.Info("GetPersistentCallersFromUnityEvent", "Found", methodName, "for", target, "with Type",
                        targetType);
        ...
        /// <summary>
        ///     Adds all UnityActions from a UnityEvent that are assigned to it in the Inspector to another UnityEvent
        /// </summary>
        /// <param name="unityEvent"></param>
        /// <param name="persistentCallers"></param>
        public static void AddPersistentCallersToUnityEvent(this UnityEvent unityEvent,
                                                            IEnumerable<UnityAction> persistentCallers)
        {
            foreach (var persistentCaller in persistentCallers)
            {
                for (var i = 0; i < unityEvent.GetPersistentEventCount(); i++)
                {
                    if (unityEvent.GetPersistentTarget(i) == (Object) persistentCaller.Target &&
                        unityEvent.GetPersistentMethodName(i) == persistentCaller.Method.Name)
                    {
                        Log.Info("AddPersistentCallersToUnityEvent",
                            "Already exists in the UnityEvent you're trying to add it to");
                ...
                #if UNITY_EDITOR
                Log.Info("AddPersistentCallersToUnityEvent", "Not yet in list, will add");
                UnityEventTools.AddPersistentListener(unityEvent, persistentCaller);
                #else
                Log.Error("AddPersistentCallersToUnityEvent", "Cannot call this outside of the editor");
                #endif
            }
        }
        ...
        /// <summary>
        ///     Adds a UnityAction from a UnityEvent that is assigned to it in the Inspector to another UnityEvent
        /// </summary>
        /// <param name="unityEvent"></param>
        /// <param name="persistentCaller"></param>
        public static void AddPersistentCallersToUnityEvent(this UnityEvent unityEvent, UnityAction persistentCaller)
        {
            for (var i = 0; i < unityEvent.GetPersistentEventCount(); i++)
            {
                if (unityEvent.GetPersistentTarget(i) == (Object) persistentCaller.Target &&
        ...
        public static void SafeFindObjectOfType<T>(this MonoBehaviour monoBehaviour, out T foundObject)
            where T : MonoBehaviour
        {
            foundObject = Object.FindObjectOfType<T>();

            if (foundObject == null)
            {
                Log.Error("SafeFindObjectOfType", "Could not find", typeof(T), "in the scene");
            }
        }
        ...
        public static void PlayEnsured(this AudioSource original, AudioClip clip)
        {
            ...
            {
                tempSource.SetCustomCurve(AudioSourceCurveType.CustomRolloff,
