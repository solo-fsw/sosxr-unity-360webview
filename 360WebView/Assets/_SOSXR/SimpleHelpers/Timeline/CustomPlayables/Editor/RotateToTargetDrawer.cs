...
public class RotateToTargetDrawer : PropertyDrawer
{
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        clipTemplate.AxisToUse = EditorGUILayout.Vector3IntField(
            new GUIContent("Axis to use", "Which axis to use for calculations? 0 = don't use, 1 = use"),
            clipTemplate.AxisToUse);
        ...
        EditorGUILayout.LabelField(
            new GUIContent(new StringBuilder("Rounded to " + roundedTo + " decimals").ToString()),
            new GUIStyle {alignment = TextAnchor.UpperRight});

        EditorGUILayout.Vector3Field(new GUIContent("Displacement from Target", "The displacement"),
            clipTemplate.DisplacementFromTarget.RoundCorrectly());

        EditorGUILayout.Vector3Field(new GUIContent("Direction to Target", "The direction to target"),
            clipTemplate.DirectionToTarget.RoundCorrectly());

        EditorGUILayout.FloatField(new GUIContent("Distance to Target", "Distance to Target"),
            clipTemplate.DistanceToTarget.RoundCorrectly());
