...
public class ToTargetDrawer : PropertyDrawer
{
    ...
    private static void CreateChangeableValuesBox(ToTargetBehaviour clipTemplate)
    {
        ...
        clipTemplate.stoppingDistance =
            EditorGUILayout.Slider("Stopping Distance", clipTemplate.stoppingDistance, 0f, 5f);
    ...
    private static void ShowValuesForLabels(ToTargetClip clip)
    {
        ...
        EditorGUILayout.LabelField(Math.Round(clip.template.startingDistance, RoundDecimal) + Meters);
        EditorGUILayout.LabelField(Math.Round(clip.template.startMinStopDistance, RoundDecimal) + Meters);

        CreateBoldLabelWithSpaceAbove(10, "");
        EditorGUILayout.LabelField(Math.Round(clip.template.durationToTarget, RoundDecimal) + Seconds);

        CreateBoldLabelWithSpaceAbove(10, "");
        EditorGUILayout.LabelField(Math.Round(clip.template.remainingDistance, RoundDecimal) + Meters);
        EditorGUILayout.LabelField(Math.Round(clip.template.remainingMinStopDistance, RoundDecimal) + Meters);
