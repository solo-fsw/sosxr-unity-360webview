...
public class TimelineSpeedDrawer : PropertyDrawer
{
    ...
    private static void DrawStartSpeedVariables(TimelineSpeedBehaviour clipTemplate)
    {
        ...
        clipTemplate.setSpeedAtStart =
            EditorGUILayout.ToggleLeft("Set speed at clip start", clipTemplate.setSpeedAtStart);

        if (clipTemplate.setSpeedAtStart)
        {
            clipTemplate.speedAtStart =
                EditorGUILayout.Slider("Speed at start", (float) clipTemplate.speedAtStart, 0f, 1f);
        }
    ...
    {
        if ((!clipTemplate.setSpeedAtStart || !(Math.Abs(clipTemplate.speedAtStart - 1) > 0.01f)) &&
            (!clipTemplate.setSpeedAtEnd || !(Math.Abs(clipTemplate.speedAtEnd - 1) > 0.01f)))
        {
            return;
        }
