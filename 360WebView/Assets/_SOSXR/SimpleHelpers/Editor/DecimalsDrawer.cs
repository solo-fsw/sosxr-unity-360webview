...
namespace mrstruijk
{
    ...
    public class DecimalsDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            {
                property.floatValue = EditorGUILayout.FloatField(new GUIContent(property.name),
                    property.floatValue.RoundCorrectly(_attribute.Decimals));
            }
            else if (property.propertyType is SerializedPropertyType.Vector2 or SerializedPropertyType.Vector3)
            {
                EditorGUI.LabelField(position, label.text,
                    "Currently Decimals only works with floats. Vector2 / Vector3 is yet to be implemented.");
            }
            else
            {
