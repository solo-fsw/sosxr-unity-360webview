...
namespace mrstruijk
{
    ...
    public abstract class EditorGUIHelpers : Editor
    {
        ...
        {
            _smallFontButtonStyle =
                new GUIStyle(GUI.skin
                                .button) // Make sure to set it as 'NEW guistyle', otherwise settings will influence the 'base' GUIStyle
                {
                    fontSize = 10
                };
        }
        ...
        private static void CreateScriptField<T>(T editorScriptTarget)
        {
            ...
            {
                EditorGUILayout.ObjectField("Custom Editor for Script:",
                    MonoScript.FromMonoBehaviour(editorScriptTarget as MonoBehaviour), editorScriptTarget.GetType(),
                    false);
            }
            else if (editorScriptTarget as ScriptableObject == true)
            {
                EditorGUILayout.ObjectField("Custom Editor for Script:",
                    ...
                    editorScriptTarget.GetType(), false);
            }
            else
            {
                Debug.LogWarning(
                    "Class does neither derive from MonoBehaviour nor ScriptableObject, therefore I don't know how to draw the 'Script' field");
            }
        ...
        protected bool CreateHeaderToggle(string propertyName, string titleName)
        {
            ...
            serializedProperty.boolValue = EditorGUILayout.ToggleLeft("", serializedProperty.boolValue,
        ...
        protected void CreateFloatField(string propertyName, string toolTip = "")
        {
            ...
            serializedProperty.floatValue = EditorGUILayout.FloatField(new GUIContent(serializedProperty.name, toolTip),
        ...
        protected void CreateFloatSliderProperty(string propertyName, string toolTip = "", float minValue = 0f,
                                                 float maxValue = 10f)
        {
            var serializedProperty = serializedObject.FindProperty(propertyName);

            serializedProperty.floatValue = EditorGUILayout.Slider(new GUIContent(serializedProperty.name, toolTip),
                serializedProperty.floatValue, minValue, maxValue);
        }
        ...
        protected void CreateIntField(string propertyName, string toolTip = "")
        {
            ...
            serializedProperty.intValue = EditorGUILayout.IntField(new GUIContent(serializedProperty.name, toolTip),
                serializedProperty.intValue);
        }


        protected void CreateIntSliderProperty(string propertyName, string toolTip = "", int minValue = 0,
                                               int maxValue = 10)
        {
            var serializedProperty = serializedObject.FindProperty(propertyName);

            serializedProperty.intValue = EditorGUILayout.IntSlider(new GUIContent(serializedProperty.name, toolTip),
                serializedProperty.intValue, minValue, maxValue);
        }


        protected void CreateAnimationCurveField(string propertyName, string toolTip = "", int start = 0,
                                                 float startValue = 0f, int end = 1, float endValue = 1f)
        {
            var serializedProperty = serializedObject.FindProperty(propertyName);

            var defaultAnimationCurve =
                new AnimationCurve(new Keyframe(start, startValue), new Keyframe(end, endValue));
        ...
        protected void CreateVector3Field(string propertyName, string toolTip = "")
        {
            ...
            serializedProperty.vector3Value =
                EditorGUILayout.Vector3Field(new GUIContent(serializedProperty.name, toolTip),
                    serializedProperty.vector3Value);
        }
        ...
        protected void CreatePropertyField(string propertyName, string toolTip = "", bool includeChildren = true)
        {
            ...
            EditorGUILayout.PropertyField(serializedProperty, new GUIContent(serializedProperty.name, toolTip),
                includeChildren);
        }
        ...
        protected SerializedProperty CreateToggleProperty(string propertyName, string toolTip = "")
        {
            ...
            serializedProperty.boolValue = EditorGUILayout.Toggle(new GUIContent(serializedProperty.name, toolTip),
        ...
        protected void CreateColorField(string propertyName, bool hdr = true, string toolTip = "")
        {
            ...
            serializedProperty.colorValue = EditorGUILayout.ColorField(new GUIContent(serializedProperty.name, toolTip),
                serializedProperty.colorValue, true, true, hdr);
        }


        protected void CreateObjectField(string propertyName, Type type, string toolTip = "",
                                         bool allowSceneObjects = true)
        {
            var serializedProperty = serializedObject.FindProperty(propertyName);

            serializedProperty.objectReferenceValue = EditorGUILayout.ObjectField(
                new GUIContent(serializedProperty.name, toolTip), serializedProperty.objectReferenceValue, type,
                allowSceneObjects);
        }
        ...
        protected void CreateStringField(string propertyName, string toolTip = "")
        {
            ...
            serializedProperty.stringValue = EditorGUILayout.TextField(new GUIContent(serializedProperty.name, toolTip),
