...
public class DeviceDependentUnityEvents : MonoBehaviour
{
    ...
    [Tooltip(
        "This can work if it is not in the same Scene as where the DeviceChecker is, since that one SETS the device on Start")]
    [SerializeField] private bool m_runOnStart = true;
    [Tooltip(
        "Since the Device is set in the DeviceChecker on Start, we might miss the event if we're not subscribed to it.")]
    ...
    private void Awake()
    {
        ...
        {
            this.Warning("Are you sure we want to run this on Start AND subscribe to events?");
        }

        if (!m_runOnStart && !m_subscribeToEvents)
        {
            this.Warning("Since neither bool is set, the UnityEvents will not be fired. Is this intentional?");
        }
    }
    ...
    private void OnEnable()
    {
        ...
        {
            EventsSystem.DeviceHasBeenSet += SendUnityEvents;
        }
    }
    ...
    private void SendUnityEvents()
    {
        ...
        {
            this.Info("CurrentDevice is", m_targetDevice, "so running event.");
            m_eventIfOnTargetDevice?.Invoke();
        }
        else if (m_currentDevice.Current != m_targetDevice)
        {
            this.Info("CurrentDevice is not", m_targetDevice, "so running event.");
            m_eventIfNotOnTargetDevice?.Invoke();
        }
        else if (m_currentDevice.Current == Device.None)
        {
            this.Error("CurrentDevice has not yet been set.");
        }
        else
        {
